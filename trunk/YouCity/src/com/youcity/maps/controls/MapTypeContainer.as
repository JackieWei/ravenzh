...
//			_transParent3DContainer.buttonMode = true;
		...
		private function drawMapType():void
		{
			...
			if (true)
			{
				...
				_measureContainer.addEventListener(MouseEvent.CLICK, measuareHandler);
...
//			}
		...
		private function naviToTransParentHandler(event:MouseEvent):void
		{
			...
			if (target.selected)
				_control.setMapType(MapTypeConstants.getManattanTransparent3DMap());
			else
				_control.setMapType(MapTypeConstants.getManhattan3DMap());
		}
		...
		private function measuareHandler(event:MouseEvent):void
		{
			if (_enclosure.selected) return;
			_measureing = !_measureing;
			if (_measureing)
				_positionControl.measureStart();
		...
		{
			this.x = map.size.x - _controlWidth - _right;
			this.y = _y;
		}
