...
{
    private var _brokenImage:String = Constants.DEFAULT_BROKEN_IMAGE;
    public function set brokenImage(value:String):void
    {
    	if(AuxUtil.isEmpty(value)) return;
        _brokenImage = value;
    }
     ...
     *  @private
     */
    private var __brokenImage:Boolean = false;
     ...
     *  @private
     */
    private var __brokenImageBorder:IFlexDisplayObject;
                                                  ...
                                                  unscaledHeight:Number):void
    {
        ...
        if (isContentLoaded)
        {
            ...
            if (_scaleContent && !__brokenImage)
                doScaleContent();
            else
                doScaleLoader();
        ...
        if (__brokenImage && !__brokenImageBorder)
        {
            var skinClass:Class = getStyle("__brokenImageBorderSkin");
            if (skinClass)
            {
                __brokenImageBorder = IFlexDisplayObject(new skinClass());
                if (__brokenImageBorder is ISimpleStyleClient)
                    ISimpleStyleClient(__brokenImageBorder).styleName = this;
                addChild(DisplayObject(__brokenImageBorder));
            }
        }
        else if (!__brokenImage && __brokenImageBorder)
        {
            removeChild(DisplayObject(__brokenImageBorder));
            __brokenImageBorder = null;
        }

        if (__brokenImageBorder)
            __brokenImageBorder.setActualSize(unscaledWidth, unscaledHeight);
    }
                            ...
                            event:IOErrorEvent):void
    {
        ...
        source = getStyle("__brokenImageSkin");

        // Force the load of the broken image skin here, since that will
        // clear the __brokenImage flag. After the image is loaded we set
        // the __brokenImage flag.
        load();
        contentChanged = false;
        __brokenImage = true;
