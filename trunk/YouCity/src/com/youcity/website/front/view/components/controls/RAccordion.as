...
[Exclude(name="defaultmx.controls.Button", kind="property")]
...
public class RAccordion extends Container implements IHistoryManagerClient, IFocusManagerComponent
{
    //--------------------------------------------------------------------------
    //
    //  Class constants
     ...
     *  Constructor.
     */
    public function RAccordion()
    {
        super();

        headerRenderer = new ClassFactory(RAccordionHeader);
    ...
    override public function get baselinePosition():Number
    {
        ...
        var header0:mx.controls.Button = getHeaderAt(0);
     ...
     *  created object must be a subclass of mx.controls.Button and implement the
                                           ...
                                           newIndex:int):void
    {
        ...
        var oldSelectedHeader:mx.controls.Button = getHeaderAt(selectedIndex);
        ...
        var newSelectedHeader:mx.controls.Button = getHeaderAt(selectedIndex);
    ...
    private function shuffleHeaders(oldIndex:int, newIndex:int):void
    {
        ...
        var header:mx.controls.Button = getHeaderAt(oldIndex);
        if (newIndex < oldIndex)
        {
    ...
    override protected function measure():void
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            var button:mx.controls.Button = getHeaderAt(i);
     ...
     *
     *  @return mx.controls.Button object representing the navigator button.
     */
    public function getHeaderAt(index:int):mx.controls.Button
    {
        return mx.controls.Button(rawChildren.getChildByName(HEADER_NAME_BASE + index));
    }
    ...
    private function createHeader(content:DisplayObject, i:int):void
    {
        ...
        var header:mx.controls.Button = mx.controls.Button(headerRenderer.newInstance());
    ...
    {
//        var header:mx.controls.Button = mx.controls.Button(event.currentTarget);
...
//            dispatchChangeEvent(oldIndex, newIndex, event);
    ...
    private function childAddHandler(event:ChildExistenceChangedEvent):void
    {
        ...
        if (numChildren == 1 && proposedSelectedIndex == -1)
        {
            ...
            // Select the new header.
            var newHeader:mx.controls.Button = getHeaderAt(0);
    ...
    private function childRemoveHandler(event:ChildExistenceChangedEvent):void
    {
        ...
        else if (index == selectedIndex)
        {
            ...
            // Select the new selected index header.
            var newHeader:mx.controls.Button = getHeaderAt(newIndex);
            newHeader.selected = true;
    ...
    private function labelChangedHandler(event:Event):void
    {
        ...
        var header:mx.controls.Button = getHeaderAt(childIndex);
