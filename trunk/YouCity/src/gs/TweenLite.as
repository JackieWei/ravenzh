	...
	public class TweenLite {
		...
		public function complete($skipRender:Boolean = false):void {
			if (!$skipRender) {
				...
				this.startTime = currentTime - (this.duration * 1000) / _timeScale;
			...
			if (this.vars.persist != true) {
				removeTween(this, false); //moved above the onComplete callback in case there's an error in the user's onComplete - this prevents constant errors
			}
		...
		public static function removeTween($t:TweenLite, $clear:Boolean = true):void {
			if ($t != null && masterList[$t.target] != undefined) {
				...
				masterList[$t.target][$t] = null; //prevents garbage collection issues.
				delete masterList[$t.target][$t];
			}
		}
		...
		public static function killTweensOf($tg:Object = null, $complete:Boolean = false):void {
			if ($tg != null && masterList[$tg] != undefined) {
				var o:Object = masterList[$tg];
				for (var tw:* in o) {
					if ($complete) {
						o[tw].complete(false);
					}
					o[tw].clear(); //prevents situations where a tween is killed but is still referenced elsewhere and put back in the render queue, like if a TweenLiteGroup is paused, then the tween is removed, then the group is unpaused.
				}
