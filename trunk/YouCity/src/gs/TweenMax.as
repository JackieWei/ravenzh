...
package gs {
	...
	public class TweenMax extends TweenFilterLite implements IEventDispatcher {
		public static var version:Number = 3.0;
		...
		public function TweenMax($target:Object, $duration:Number, $vars:Object) {
			...
			if (TweenFilterLite.version < 9.0) {
		...
		public function resume():void {
			if (_pauseTime != -1) {
				var d:Number = this.delay * (1000 / _combinedTimeScale); //delay in milliseconds
				var started:Boolean = (_pauseTime > this.initTime + d);
				this.initTime += currentTime - _pauseTime;
		...
		public function restart($includeDelay:Boolean=false):void {
			if ($includeDelay) {
				...
				this.startTime = currentTime + (this.delay * (1000 / _combinedTimeScale));
			} else {
				this.startTime = currentTime;
				this.initTime = currentTime - (this.delay * (1000 / _combinedTimeScale));
			}
		...
		public function reverse($adjustDuration:Boolean=true):void {
			...
			if (p < 1) {
				...
				TweenLite.removeTween(this, false);
			} else {
				pause();
			}
			
			if ($adjustDuration && p > 0) {
				this.startTime = currentTime - ((1 - p) * this.duration * 1000 / _combinedTimeScale);
				this.initTime = this.startTime - (this.delay * (1000 / _combinedTimeScale));
			}
		}
		...
		protected function enable():void { //just makes sure it's in the render queue. For example, if persist wasn't set to true and the tween already completed, it gets taken out of the rendering queue, but if the user kept track of it in a variable and wants to restart() or reverse() or resume(), we need a way to make sure it's back in the queue.
			...
			if (masterList[this.target][this] != this) {
				_combinedTimeScale = _timeScale * _globalTimeScale;
		...
		public function invalidate($adjustStartValues:Boolean=true):void { //forces the vars to be re-parsed and immediately re-rendered
			if (this.initted) {
				...
				_subTweens = [];
				initTweenVals();
				_combinedTimeScale = (this.vars.timeScale || 1) * _globalTimeScale;
				if (this.vars.onCompleteListener != null || this.vars.onUpdateListener != null || this.vars.onStartListener != null) {
					if (_dispatcher != null) {
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		...
		public static function sequence($target:Object, $tweens:Array):Array { 
			...
			return [];
			/* temporarily leave the deprecated code here in case someone needs it for backwards-compatibility...
		...
		public static function getTweensOf($target:Object):Array {
			var t:Dictionary = masterList[$target];
			var a:Array = [];
			if(t != null) {
		...
		public static function getAllTweens():Array {
			var a:Dictionary = masterList; //speeds things up slightly
...
//---- GETTERS / SETTERS ----------------------------------------------------------------------------------------------------------
		...
		public function get progress():Number {
			var t:Number = (_pauseTime != -1) ? _pauseTime : currentTime;
			var p:Number = (((t - this.initTime) / 1000) - this.delay / _combinedTimeScale) / this.duration * _combinedTimeScale;
			if (p > 1) {
				return 1;
			} else if (p < 0) {
				return 0;
